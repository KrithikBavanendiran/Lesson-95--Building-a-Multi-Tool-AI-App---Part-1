...
ENHANCE_SYS=(
    ...
    "Add subjects, style, lighting, camera angle, background, colors. Keep it safe."
)
...
def gen_image(prompt: str):
    ...
    try:
        ...
        if "megative_prompt" in msg or "unexpected keyword" in msg:
            try:
                return img_client.text_to_image(
                    prompt=prompt,
                    model=MODEL_ID,
...
def main():
    ...
    st.info("Flow: Enter a prompt → Enhance prompt → Safety filter → Generate image.")    
    ...
    if submit:
        ...
        if not raw_check.get("ok"):
            st.error(f"Prompt blocked by safety filter: {raw_check.get('reason', 'Unsafe prompt')}")
            return
        ...
        if not enhanced_check.get("ok"):
            st.error(f"Enhanced prompt blocked by safety filter: {enhanced_check.get('reason', 'Unsafe prompt')}")
            return
